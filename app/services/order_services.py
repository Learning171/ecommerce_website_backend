...
async def create_order(db: db_dependency, order_create: OrderCreate):
...
async def get_order(db: db_dependency, order_id: int):
    return db.query(Order).filter(Order.id == order_id).first()


async def get_orders(db: db_dependency, skip: int = 0, limit: int = 10):
