...
async def get_all_products(db: db_dependency, skip: int = 0, limit: int = 10):
    return (
        db.query(Product)
...
async def create_product(db: db_dependency, product_create: ProductCreate):
...
async def update_product(db: db_dependency, product_id: int, product_update: ProductCreate):
...
async def create_image_record(db: db_dependency, product_id: int, filename: str):
...
async def update_image_record(db: db_dependency, image_id: int, filename: str) -> Image:
...
async def validate_image_extension(file_name: str):
