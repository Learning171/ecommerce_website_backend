...
async def add_to_cart(db: db_dependency, cart_create: CartCreate):
    existing_cart_item = (
...
async def update_cart_item_quantity(db: db_dependency, cart_id: int, new_quantity: int):
...
async def remove_from_cart(db: db_dependency, cart_id: int):
    db_cart_item = db.query(Cart).filter(Cart.id == cart_id).first()
