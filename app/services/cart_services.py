...
def add_to_cart(db: db_dependency, cart_create: CartCreate):
    existing_cart_item = db.query(Cart).filter(Cart.product_id == cart_create.product_id).first()
    existing_product_price = db.query(Product).filter(Product.id == cart_create.product_id).first()
    
    if existing_cart_item:
        total_price = existing_product_price.price * existing_cart_item.quantity
...
def update_cart_item_quantity(db: db_dependency, cart_id: int, new_quantity: int, cart_update: CartCreate):
    db_cart_item = db.query(Cart).filter(Cart.id == cart_id).first()
    existing_product_price = db.query(Product).filter(Product.id == cart_update.product_id).first()
