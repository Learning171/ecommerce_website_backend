...
def add_to_cart(db: db_dependency, cart_create: CartCreate):
    ...
    if existing_cart_item:
        ...
        raise ValueError("Item already exists in the cart. Update the quantity if needed.")
...
def update_cart_item_quantity(db: db_dependency, cart_id: int, new_quantity: int):
    ...
    existing_product_price = db.query(Product).filter(Product.id == db_cart_item.product_id).first()
