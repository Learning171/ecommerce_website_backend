...
async def read_all_reviews_for_product(
    product_id: int, db: db_dependency, skip: int = 0, limit: int = 10
):
    return get_all_reviews_for_product(db, product_id, skip=skip, limit=limit)


@review_router.get("/reviews/{product_id}/{review_id}")
async def read_review_for_product(product_id: int, review_id: int, db: db_dependency):
...
# @review_router.post("/reviews/{product_id}")
# async def create_new_review_for_product(review_create: ReviewCreate, db: db_dependency, user: cutomer_required):
#     return create_review_for_product(db, review_create)
...
# @review_router.get("/reviews/{product_id}/{review_id}")
# async def read_review_for_product(product_id: int, review_id: int, db: db_dependency, user: cutomer_required):
#     review = get_review_for_product(db, product_id, review_id)
#     if review is None:
#         raise HTTPException(
#             status_code=404,
