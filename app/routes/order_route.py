...
async def create_new_order(order_create: OrderCreate, db: db_dependency):
...
# @order_router.post("/orders/")
# async def create_new_order(order_create: OrderCreate, db: db_dependency, user: cutomer_required):
#     return create_order(db, order_create)


# @order_router.get("/orders/{order_id}")
# async def read_order(order_id: int, db: db_dependency, user: cutomer_required):
#     order = get_order(db, order_id)
#     if order is None:
#         raise HTTPException(status_code=404, detail="Order not found")
#     return order
...
# @order_router.get("/orders/")
# async def read_orders(db: db_dependency, user: cutomer_required, skip: int = 0, limit: int = 10):
#     return get_orders(db, skip=skip, limit=limit)
