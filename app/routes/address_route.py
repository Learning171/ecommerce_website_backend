...
def create_address(address: AddressCreate, db: db_dependency):
...
async def update_address(
    address_id: int, address: AddressCreate, db: db_dependency):
    db_address = db.query(Address).filter(Address.id == address_id).first()
    if db_address is None:
        raise HTTPException(status_code=404, detail="Address not found")
...
# @address_router.get("/addresses/{address_id}", response_model=AddressResponse)
# async def read_address(address_id: int, db: db_dependency, user: cutomer_required):
#     db_address = db.query(Address).filter(Address.id == address_id).first()
#     if db_address is None:
#         raise HTTPException(status_code=404, detail="Address not found")
#     return db_address
...
# @address_router.delete("/addresses/{address_id}", response_model=AddressResponse)
# async def delete_address(address_id: int, db: db_dependency, user: cutomer_required):
#     db_address = db.query(Address).filter(Address.id == address_id).first()
#     if db_address is None:
#         raise HTTPException(status_code=404, detail="Address not found")
...
# @address_router.get("/addresses/", response_model=AddressList)
# async def list_addresses(db: db_dependency, user: cutomer_required):
#     addresses = db.query(Address).all()
#     total = db.query(Address).count()
#     return {"items": addresses, "total": total}
