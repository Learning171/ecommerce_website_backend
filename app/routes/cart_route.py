...
async def add_item_to_cart(
    cart_create: CartCreate, db: db_dependency):
...
async def update_cart_quantity(cart_id: int, new_quantity: int, db: db_dependency):
...
async def remove_item_from_cart(cart_id: int, db: db_dependency):
...
# @cart_router.get("/cart/")
# async def get_cart_items(db: db_dependency, user:cutomer_required):
#     return get_user_cart(db)


# @cart_router.put("/cart/{cart_id}")
# async def update_cart_quantity(cart_id: int, new_quantity: int, db: db_dependency, user:cutomer_required):
#     cart_item = update_cart_item_quantity(db, cart_id, new_quantity)
#     if not cart_item:
#         raise HTTPException(status_code=404, detail="Cart item not found")
#     return cart_item
...
# @cart_router.delete("/cart/{cart_id}")
# async def remove_item_from_cart(cart_id: int, db: db_dependency, user:cutomer_required):
#     cart_item = remove_from_cart(db, cart_id)
#     if not cart_item:
#         raise HTTPException(status_code=404, detail="Cart item not found")
#     return cart_item
